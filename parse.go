...
// Security limits for parser buffers to prevent unbounded memory growth
// from malicious or misbehaving terminal input.
const (
	// maxOscDataLen is the maximum number of runes stored in an OSC data
	// payload. Excess data is silently discarded.
	maxOscDataLen = 4096

	// maxIntermediateLen is the maximum number of intermediate characters
	// stored for CSI, ESC, and DCS sequences. ECMA-48 doesn't define any
	// control sequences with more than two intermediates.
	maxIntermediateLen = 16

	// maxParamsLen is the maximum number of raw parameter runes (digits,
	// semicolons, colons) stored before dispatch. This bounds the memory
	// used by a single CSI or DCS parameter string.
	maxParamsLen = 1024

	// maxCSIParams is the maximum number of parsed integer parameters
	// retained after splitting the raw parameter string. ECMA-48
	// specifies that a maximum of 16 parameters need be stored.
	maxCSIParams = 16
)
...
// behaviour in later Session series, as described in ground.
...
func (p *Parser) oscPut(r rune) {
	if len(p.oscData) >= maxOscDataLen {
		return
	}
