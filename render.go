...
// Options configures how terminal content is rendered to an image.
type Options struct {
	// Font is the monospaced font face used to draw characters.
	// Defaults to basicfont.Face7x13 (7×13 bitmap font).
	Font font.Face

	// FontBold, when set, is used for cells with the bold (SGR 1) attribute.
	// If nil, bold is synthesized by drawing the glyph twice with a +1px offset.
	FontBold font.Face

	// FontItalic, when set, is used for cells with the italic (SGR 3) attribute.
	// If nil, italic is synthesized by shearing the glyph.
	FontItalic font.Face

	// FontBoldItalic, when set, is used for cells with both bold and italic.
	FontBoldItalic font.Face

	// CellWidth is the width of a single character cell in pixels.
	// 0 means derive from Font (via GlyphAdvance of a space).
	CellWidth int

	// CellHeight is the height of a single character cell in pixels.
	// 0 means derive from Font metrics.
	CellHeight int

	// FgDefault is the foreground color used when a cell specifies
	// tcellcolor.Default. Defaults to light gray (#CCCCCC).
	FgDefault color.Color

	// BgDefault is the background color used when a cell specifies
	// tcellcolor.Default. Defaults to near-black (#0D0D0D).
	BgDefault color.Color

	// Padding is the number of pixels of empty space added around the
	// terminal content on all sides. Defaults to 8.
	Padding int

	// Scale multiplies all pixel dimensions (cell size, padding, font size).
	// Default 1.  Use 2 or 3 for high-DPI output.
	Scale int

	// RenderCursor, when true, draws a cursor indicator at the current
	// cursor position.
	RenderCursor bool

	// CursorStyle selects the cursor shape: block (default), underline, or bar.
	// CursorStyleBlock/CursorStyleUnderline/CursorStyleBar.  For block cursors,
	// the cell is inverted (fg↔bg) so the glyph remains legible.
	CursorStyle tcell.CursorStyle

	// CursorColour is the colour used for the cursor indicator for underline/bar
	// shapes.  For block cursors, it is ignored (inversion is used instead).
	// Defaults to white.
	CursorColour color.Color

	// Scheme, when non-nil, overrides the RGB values used for the 16 (or up
	// to 256) ANSI palette entries and, when its Foreground/Background/Cursor
	// fields are set, the default foreground, background, and cursor colors.
	// Scheme colors take precedence over FgDefault/BgDefault/CursorColour.
	Scheme *Scheme

	// DimFactor controls how much the foreground is blended toward the
	// background for dim (SGR 2) cells.  0 = no blending (fg unchanged),
	// 1 = fully blended to background.  Default 0.55.
	DimFactor float64

	// BoldAsBright, when true, promotes bold indexed colors (SGR 1 + 30–37)
	// to their bright counterparts (90–97).  Default false.
	BoldAsBright bool

	// DrawBoxChars, when true, renders box-drawing and block-element
	// characters (U+2500–259F), braille (U+2800–28FF), and similar glyphs
	// procedurally instead of via the font.  This ensures perfect tiling
	// without gaps or seams.  Default true.
	DrawBoxChars bool
}

// DefaultRenderOptions returns sensible defaults for PNG/GIF rendering.
func DefaultRenderOptions() Options {
	return Options{
		Font:         basicfont.Face7x13,
		CellWidth:    7,
		CellHeight:   13,
		FgDefault:    color.RGBA{R: 0xCC, G: 0xCC, B: 0xCC, A: 0xFF},
		BgDefault:    color.RGBA{R: 0x0D, G: 0x0D, B: 0x0D, A: 0xFF},
		Padding:      8,
		Scale:        1,
		DimFactor:    0.55,
		DrawBoxChars: true,
...
// applyRenderDefaults fills zero-valued fields in opts with defaults.
func applyRenderDefaults(opts *Options) {
	if opts.Font == nil {
		opts.Font = basicfont.Face7x13
	}
	if opts.CellWidth <= 0 {
		if adv, ok := opts.Font.GlyphAdvance(' '); ok {
			opts.CellWidth = adv.Ceil()
		}
		if opts.CellWidth <= 0 {
			opts.CellWidth = 7
		}
	...
	if opts.CellHeight <= 0 {
		m := opts.Font.Metrics()
		opts.CellHeight = m.Height.Ceil()
		if opts.CellHeight <= 0 {
			opts.CellHeight = 13
		}
	}
	if opts.FgDefault == nil {
		opts.FgDefault = color.RGBA{R: 0xCC, G: 0xCC, B: 0xCC, A: 0xFF}
	}
	if opts.BgDefault == nil {
		opts.BgDefault = color.RGBA{R: 0x0D, G: 0x0D, B: 0x0D, A: 0xFF}
	}
	if opts.Scale <= 0 {
		opts.Scale = 1
	}
	...
	// Apply scale to cell dimensions and padding.
	if opts.Scale > 1 {
		opts.CellWidth *= opts.Scale
		opts.CellHeight *= opts.Scale
		opts.Padding *= opts.Scale
	}
}

// tcellColorToRGBA converts a tcell.Color to an image/color.Color.
// When c is invalid or ColorDefault the fallback is returned instead.
func tcellColorToRGBA(c tcell.Color, fallback color.Color) color.Color {
	tc := c.TrueColor()
	if tc == tcellcolor.Default || !tc.Valid() {
		return fallback
	}
	r, g, b := tc.RGB()
	if r < 0 {
...
// resolveColor maps a tcell.Color to an image/color.Color, applying a colour
// scheme lookup (if any) before falling back to tcell's built-in palette.
// ColorDefault resolves to the supplied fallback (the scheme's default fg/bg).
func resolveColor(c tcell.Color, fallback color.Color, lookup map[tcell.Color]color.Color) color.Color {
	if c == tcellcolor.Default {
		return fallback
	}
	if lookup != nil {
		if col, ok := lookup[c]; ok {
			return col
		}
	}
	return tcellColorToRGBA(c, fallback)
}

// renderCells is the shared rendering kernel.  It draws a grid of Cell values
// to a new *image.RGBA and returns it.  When cursorCol/cursorRow are non-negative
// and opts.RenderCursor is true, a cursor indicator is drawn.
func renderCells(cells [][]Cell, opts Options, cursorCol, cursorRow int) *image.RGBA {
	applyRenderDefaults(&opts)

	scheme := opts.Scheme
	lookup := scheme.lookup()
	defFg, defBg := opts.FgDefault, opts.BgDefault
	if scheme != nil {
		if scheme.Foreground != nil {
			defFg = scheme.Foreground
		}
		if scheme.Background != nil {
			defBg = scheme.Background
		}
	}
	...
	draw.Draw(img, img.Bounds(), image.NewUniform(defBg), image.Point{}, draw.Src)

	metrics := opts.Font.Metrics()
	...
	for y := 0; y < rows; y++ {
		for x := 0; x < cols; {
			...
			fg := resolveColor(c.Style.GetForeground(), defFg, lookup)
			bg := resolveColor(c.Style.GetBackground(), defBg, lookup)

			// Bold-as-bright: promote indexed fg 30–37 → 90–97.
			if opts.BoldAsBright && c.Style.HasBold() {
				fg = brightenIndexed(fg)
			}
			if c.Style.HasReverse() {
				fg, bg = bg, fg
			}

			// Dim: blend fg toward bg.
			if c.Style.HasDim() && opts.DimFactor > 0 {
				fg = blendRGBA(fg, bg, opts.DimFactor)
			}
			...
			cellRect := image.Rect(
				pad+x*cw, pad+y*ch,
				pad+(x+w)*cw, pad+(y+1)*ch,
			)
			draw.Draw(img, cellRect, image.NewUniform(bg), image.Point{}, draw.Src)

			// Pick the right face for style combinations.
			face := opts.Font
			if c.Style.HasBold() && c.Style.HasItalic() && opts.FontBoldItalic != nil {
				face = opts.FontBoldItalic
			} else if c.Style.HasBold() && opts.FontBold != nil {
				face = opts.FontBold
			} else if c.Style.HasItalic() && opts.FontItalic != nil {
				face = opts.FontItalic
			}
			...
			if r != ' ' {
				glyphX := pad + x*cw
				glyphY := pad + y*ch + ascent
				if opts.DrawBoxChars && isBoxChar(r) {
					drawBoxChar(img, cellRect, r, fg, bg)
				} else {
					bd := font.Drawer{
						Dst:  img,
						Src:  image.NewUniform(fg),
						Face: face,
						Dot:  fixed.P(glyphX, glyphY),
					}
					bd.DrawString(string(r))
					// Bold synthesis: draw again +1px.
					if c.Style.HasBold() && opts.FontBold == nil {
						bd.Dot = fixed.P(glyphX+1, glyphY)
						bd.DrawString(string(r))
					}
					// Italic synthesis.
					if c.Style.HasItalic() && opts.FontItalic == nil {
						drawShearedGlyph(img, face, fixed.P(glyphX, glyphY), r, fg)
					}
					// Combining marks.
					for _, comb := range c.Combining {
						bd.Dot = fixed.P(glyphX, glyphY)
						bd.DrawString(string(comb))
					}
				}
			}

			// Underline.
			if c.Style.HasUnderline() {
				ulY := pad + (y+1)*ch - descent + 1
				drawHLine(img, pad+x*cw, pad+(x+w)*cw, ulY, fg, 1)
			}

			// Strikethrough.
			if c.Style.HasStrikeThrough() {
				stY := pad + y*ch + (ch / 2)
				drawHLine(img, pad+x*cw, pad+(x+w)*cw, stY, fg, 1)
			}

			// Overline.
			if c.Overline {
				olY := pad + y*ch + 1
				drawHLine(img, pad+x*cw, pad+(x+w)*cw, olY, fg, 1)
			}
	...
	// Draw cursor indicator if requested and position is valid.
	if opts.RenderCursor && cursorCol >= 0 && cursorRow >= 0 &&
		cursorRow < rows && cursorCol < cols {
		curX := pad + cursorCol*cw
		curY := pad + cursorRow*ch
		curW, curH := cw, ch

		curColor := opts.CursorColour
		if curColor == nil && scheme != nil && scheme.Cursor != nil {
			curColor = scheme.Cursor
		}
		if curColor == nil {
			curColor = color.White
		}

		cursorRect := image.Rect(curX, curY, curX+curW, curY+curH)
		switch opts.CursorStyle {
		case tcell.CursorStyleBlinkingUnderline, tcell.CursorStyleSteadyUnderline:
			draw.Draw(img, image.Rect(curX, curY+curH-2, curX+curW, curY+curH),
				image.NewUniform(curColor), image.Point{}, draw.Over)
		case tcell.CursorStyleBlinkingBar, tcell.CursorStyleSteadyBar:
			draw.Draw(img, image.Rect(curX, curY, curX+2, curY+curH),
				image.NewUniform(curColor), image.Point{}, draw.Over)
		default:
			// Block cursor (default, blinking, steady): invert the cell.
			invertRect(img, cursorRect)
		}
	}
...
// drawHLine fills a horizontal 1px line from x1 to x2 at row y with the given colour.
func drawHLine(img *image.RGBA, x1, x2, y int, c color.Color, thickness int) {
	for t := 0; t < thickness; t++ {
		for x := x1; x < x2; x++ {
			if x >= 0 && x < img.Bounds().Max.X && y+t >= 0 && y+t < img.Bounds().Max.Y {
				img.Set(x, y+t, c)
			}
		}
	}
...
// invertRect swaps every pixel in rect between the given colour and the existing
// pixel colour (typically fg ↔ bg for a cursor).
...
// blendRGBA blends colour a toward b by factor t (0 = all a, 1 = all b).
func blendRGBA(a, b color.Color, t float64) color.Color {
	ra, ga, ba, _ := a.RGBA()
	rb, gb, bb, _ := b.RGBA()
	blend := func(va, vb uint32) uint8 {
		return uint8((uint32(float64(va)*(1-t) + float64(vb)*t)) >> 8)
	}
	return color.RGBA{R: blend(ra, rb), G: blend(ga, gb), B: blend(ba, bb), A: 0xFF}
}

// brightenIndexed promotes an indexed foreground colour 30–37 to 90–97.
func brightenIndexed(c color.Color) color.Color {
	rgba, ok := c.(color.RGBA)
	if !ok {
		return c
	}
	// Only promote default-palette colours; leave truecolour alone.
	if rgba.R == rgba.G && rgba.G == rgba.B && rgba.R < 128 {
		return color.RGBA{R: 255, G: 255, B: 255, A: 0xFF}
	}
	return c
}

// drawShearedGlyph renders a horizontally-sheared version of r for italic
// synthesis.  It draws to a temp RGBA and copies each row shifted right by an
// increasing offset (max 2px at bottom).
func drawShearedGlyph(img *image.RGBA, face font.Face, dot fixed.Point26_6, r rune, fg color.Color) {
	// Measure via font.Drawer to get the bounding box.
	...
	tmp := image.NewRGBA(image.Rect(0, 0, w+4, h+4))
	d := font.Drawer{
		Dst:  tmp,
		Src:  image.NewUniform(fg),
		Face: face,
		Dot:  fixed.P(2-(bounds.Min.X).Ceil(), 2-(bounds.Min.Y).Ceil()),
	}
	d.DrawString(string(r))

	shearMax := 2
	dx0 := dot.X.Ceil()
	dy0 := dot.Y.Ceil() - (bounds.Min.Y).Ceil() + 2
	for y := 0; y < h+4; y++ {
		offset := shearMax * y / (h + 5)
		for x := 0; x < w+4; x++ {
			if tmp.RGBAAt(x, y).A > 0 {
				tx := dx0 + x + offset
				ty := dy0 + y
				if tx >= 0 && tx < img.Bounds().Max.X && ty >= 0 && ty < img.Bounds().Max.Y {
					img.Set(tx, ty, fg)
				}
			}
		}
...
// isBoxChar reports whether r is a procedural-renderable box/block-element
// or Braille codepoint.
func isBoxChar(r rune) bool {
	return (r >= 0x2500 && r <= 0x259F) || // box drawing + block elements
		(r >= 0x2800 && r <= 0x28FF) // braille
}

// drawBoxChar renders a single box-drawing or block-element character
// procedurally inside the given cell rectangle.
func drawBoxChar(img *image.RGBA, r image.Rectangle, ch rune, fg, bg color.Color) {
	w := r.Dx()
	h := r.Dy()
	if w < 2 || h < 2 {
		return
	...
	case 0x2580: // ▀ upper half
		draw.Draw(img, image.Rect(x0, y0, x0+w, y0+h/2), image.NewUniform(fg), image.Point{}, draw.Src)
		return
	case 0x2584: // ▄ lower half
		draw.Draw(img, image.Rect(x0, y0+h/2, x0+w, y0+h), image.NewUniform(fg), image.Point{}, draw.Src)
		return
	case 0x258C: // ▌ left half
		draw.Draw(img, image.Rect(x0, y0, x0+w/2, y0+h), image.NewUniform(fg), image.Point{}, draw.Src)
		return
	case 0x2590: // ▐ right half
		draw.Draw(img, image.Rect(x0+w/2, y0, x0+w, y0+h), image.NewUniform(fg), image.Point{}, draw.Src)
		return
	case 0x2591: // ░ light shade (25%)
		drawShade(img, r, fg, bg, 64)
		return
	case 0x2592: // ▒ medium shade (50%)
		drawShade(img, r, fg, bg, 128)
		return
	case 0x2593: // ▓ dark shade (75%)
		drawShade(img, r, fg, bg, 192)
		return
	}

	// Box drawing (U+2500–257F).
	hw := max(1, w/12) // line thickness

	// Horizontal line.
	if ch >= 0x2500 && ch <= 0x2503 && ch != 0x2502 { // ─━│┃
		drawHLine(img, x0, x0+w, y0+h/2-hw/2, fg, hw)
		return
	}
	// Vertical line.
	if ch == 0x2502 || ch == 0x2503 {
		drawVLine(img, x0+w/2-hw/2, y0, y0+h, fg, hw)
		return
	}
	...
	switch ch {
	case 0x250C: // ┌
		drawHLine(img, x0+w/2, x0+w, y0+h/2-hw/2, fg, hw)
		drawVLine(img, x0+w/2-hw/2, y0+h/2, y0+h, fg, hw)
	case 0x2510: // ┐
		drawHLine(img, x0, x0+w/2, y0+h/2-hw/2, fg, hw)
		drawVLine(img, x0+w/2-hw/2, y0+h/2, y0+h, fg, hw)
	case 0x2514: // └
		drawHLine(img, x0+w/2, x0+w, y0+h/2-hw/2, fg, hw)
		drawVLine(img, x0+w/2-hw/2, y0, y0+h/2, fg, hw)
	case 0x2518: // ┘
		drawHLine(img, x0, x0+w/2, y0+h/2-hw/2, fg, hw)
		drawVLine(img, x0+w/2-hw/2, y0, y0+h/2, fg, hw)
	case 0x251C: // ├
...
// drawVLine draws a vertical line of given thickness.
func drawVLine(img *image.RGBA, x, y1, y2 int, c color.Color, thickness int) {
	for t := 0; t < thickness; t++ {
		for y := y1; y < y2; y++ {
			if x+t >= 0 && x+t < img.Bounds().Max.X && y >= 0 && y < img.Bounds().Max.Y {
				img.Set(x+t, y, c)
			}
		}
	}
...
// drawShade fills the rect with a dithered pattern at the given alpha level.
func drawShade(img *image.RGBA, r image.Rectangle, fg, bg color.Color, level uint8) {
	// Simple checkered dither at the given level.
	for y := r.Min.Y; y < r.Max.Y; y++ {
		for x := r.Min.X; x < r.Max.X; x++ {
			col := bg
			if ((x/2 + y/2) % 2) == 0 {
				col = blendRGBA(fg, bg, float64(level)/255)
			}
			if x >= 0 && x < img.Bounds().Max.X && y >= 0 && y < img.Bounds().Max.Y {
				img.Set(x, y, col)
			}
...
// Render renders the Session's current active screen to a new *image.RGBA.
// It is safe to call from any goroutine; it takes the Session lock internally.
// The Session need not have a real Screen attached — it reads directly from the
// internal cell buffer, which is always kept up-to-date by the parse loop.
func Render(s *Session, opts Options) *image.RGBA {
	s.mu.Lock()
	h := s.height()
	w := s.width()
	snapshot := make([][]Cell, h)
	for row := 0; row < h; row++ {
		snapshot[row] = make([]Cell, w)
		for col := 0; col < w; col++ {
			src := s.activeScreen[row][col]
			var comb []rune
			if len(src.combining) > 0 {
				comb = append([]rune(nil), src.combining...)
			}
	...
	if opts.RenderCursor {
		s.mu.Lock()
		if s.mode&dectcem > 0 {
			cursorCol = int(s.cursor.col)
			cursorRow = int(s.cursor.row)
		}
		s.mu.Unlock()
	}
	return renderCells(snapshot, opts, cursorCol, cursorRow)
}

// RenderImage renders the Terminal terminal's current screen state to a new
// *image.RGBA using the provided options.  It is safe to call from any goroutine.
func (t *Terminal) RenderImage(opts Options) *image.RGBA {
	t.surface.mu.RLock()
	rows := len(t.surface.cells)
	cols := 0
	if rows > 0 {
		cols = len(t.surface.cells[0])
	}
	snapshot := make([][]Cell, rows)
	for y := 0; y < rows; y++ {
		snapshot[y] = make([]Cell, cols)
		copy(snapshot[y], t.surface.cells[y])
	}
	t.surface.mu.RUnlock()
	cursorCol, cursorRow := -1, -1
	if opts.RenderCursor {
		t.s.mu.Lock()
		if t.s.mode&dectcem > 0 {
			cursorCol = int(t.s.cursor.col)
			cursorRow = int(t.s.cursor.row)
		}
		t.s.mu.Unlock()
	}
	return renderCells(snapshot, opts, cursorCol, cursorRow)
...
// defaultFont is the font used by the package-level drawString / measureString
// convenience functions.  It defaults to Font8x8.
var defaultFont = font8x8

// spacing is the extra pixel gap inserted between consecutive glyphs (1 px by
// default).
var spacing = 1

// Drawable is the minimal interface required to render a font — any type that
// can set the colour of an individual pixel at (x, y).  *image.RGBA satisfies
// this interface out of the box.
type Drawable interface {
	Set(x, y int, c color.Color)
}

// Font is a simple fixed-width (optionally variable-width) bitmap font.
// Glyphs are packed as bitfields inside a []uint32 data slice; a charmap
// maps each rune to its offset within that slice.
type Font struct {
	charWidth    uint8
	charHeight   uint8
...
// NewFont constructs a Font from the raw character dimensions, charmap
// and data slice produced by the fontgen tool.
func NewFont(w, h uint8, cm map[rune]uint16, d []uint32) *Font {
	return &Font{w, h, cm, d, w}
}

// GetWidth returns the fixed character width of the font in pixels.
func (p *Font) GetWidth() int {
	return int(p.charWidth)
}

// GetHeight returns the character height of the font in pixels.
func (p *Font) GetHeight() int {
	return int(p.charHeight)
}

// SetVariableWidth switches between fixed-width and variable-width glyph
// advance.  In variable-width mode only the columns that actually contain set
// pixels are counted, making proportional-looking text possible.
func (p *Font) SetVariableWidth(isVar bool) {
	if !isVar {
		p.varCharWidth = p.charWidth
	} else {
		// Space is approximately 1/3 em, but at least 3 px.
...
// DrawRune draws a single rune r in colour clr at position (x, y) — the
// top-left corner of the glyph bounding box — onto dr.
//
// It returns (true, advance) if the rune is present in the font, or
// (false, fallbackAdvance) when it is not.  The caller should add the returned
// advance (plus spacing) to x before drawing the next rune.
func (p *Font) DrawRune(dr Drawable, x, y int, r rune, clr color.Color) (bool, int) {
	poff, haveChar := p.charmap[r]
	if !haveChar {
...
// drawString renders the string s in colour clr starting at (x, y) onto dr,
// advancing x by the glyph width plus spacing for every character.
// It returns the final x position after the last glyph.
func (p *Font) drawString(dr Drawable, x, y int, s string, clr color.Color) int {
	for _, c := range s {
...
// MeasureRune returns the pixel advance for a single rune without drawing.
func (p *Font) MeasureRune(r rune) (bool, int) {
	poff, haveChar := p.charmap[r]
	if !haveChar {
		return false, int(p.varCharWidth)
	}
...
// measureString returns the total pixel advance for string s without drawing.
func (p *Font) measureString(s string) int {
	x := 0
	for _, c := range s {
		_, w := p.MeasureRune(c)
		x += w + spacing
...
// drawString is a convenience wrapper that calls drawString on defaultFont.
func drawString(dr Drawable, x, y int, s string, clr color.Color) int {
	return defaultFont.drawString(dr, x, y, s, clr)
}

// measureString is a convenience wrapper that calls measureString on defaultFont.
func measureString(s string) int {
	return defaultFont.measureString(s)
}

// stringDrawable implements Drawable so pixel fonts can be rendered into a
// printable text representation (FIGlet-style).
type stringDrawable struct {
	lines [][]byte
}

// Set marks the pixel at (x, y) with 'X'.
...
// String returns a multi-line string representation of the current content.
func (s *stringDrawable) String() string {
	return s.PrefixString("")
}

// PrefixString returns the string representation with p prepended to each line.
