...
func keyCode(ev *tcell.EventKey, decckm bool) string {
	key := strings.Builder{}
	switch ev.Modifiers() {
	case tcell.ModNone:
		switch ev.Key() {
		...
		case tcell.KeyUp:
			if decckm {
				key.WriteString("\x1b[A")
			} else {
				key.WriteString(info.KeyUp)
			}
		case tcell.KeyDown:
			if decckm {
				key.WriteString("\x1b[B")
			} else {
				key.WriteString(info.KeyDown)
			}
		case tcell.KeyRight:
			if decckm {
				key.WriteString("\x1b[C")
			} else {
				key.WriteString(info.KeyRight)
			}
		case tcell.KeyLeft:
			if decckm {
				key.WriteString("\x1b[D")
			} else {
				key.WriteString(info.KeyLeft)
			}
		case tcell.KeyHome:
			if decckm {
				key.WriteString("\x1b[H")
			} else {
				key.WriteString(info.KeyHome)
			}
		case tcell.KeyEnd:
			if decckm {
				key.WriteString("\x1b[F")
			} else {
				key.WriteString(info.KeyEnd)
			}
		default:
	...
	case tcell.ModCtrl:
		...
		default:
			// tcell v3 normalises Ctrl+letter keys to KeyCtrlA..KeyCtrlZ
			// with Str()="" (empty). Convert them back to the corresponding
			// ASCII control character byte (0x01..0x1A) for the PTY.
			if ev.Key() >= tcell.KeyCtrlA && ev.Key() <= tcell.KeyCtrlZ {
				key.WriteRune(rune(ev.Key() - tcell.KeyCtrlA + 1))
			} else if s := ev.Str(); s != "" {
				key.WriteString(s)
			}
		}
